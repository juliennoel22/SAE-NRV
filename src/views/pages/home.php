<section>
    <a href="?action=register">Créer un compte</a>
    <a href="?action=login">Se connecter</a>
    <a href="?action=logout">Se déconnecter</a>
</section>
<section>
    <h2>À propos du festival</h2>
    <p>Le festival Nancy Rock Vibration (NRV) est un événement de deux semaines mettant en vedette une variété de styles de musique rock, y compris le rock classique, le blues rock, le métal et la chanson. Chaque soir, plusieurs lieux accueillent différentes performances, offrant une expérience unique aux amateurs de musique.</p>
</section>
<section>
    <h2>Programme</h2>
    <p>Découvrez le programme complet des performances, filtrez par date, lieu ou style de musique pour trouver vos spectacles préférés.</p>
    <ul>
        <li><a href="#">Programme complet</a></li>
        <li><a href="#">Par date</a></li>
        <li><a href="#">Par lieu</a></li>
        <li><a href="#">Par style de musique</a></li>
    </ul>
</section>
<section>
    <h2>Performances en vedette</h2>
    <article>
        <h3>Nuit des légendes du rock</h3>
        <p>Rejoignez-nous pour une nuit inoubliable avec des groupes de rock légendaires interprétant leurs plus grands succès.</p>
    </article>
    <article>
        <h3>Extravaganza Blues Rock</h3>
        <p>Vivez les sons envoûtants du blues rock avec des artistes de renommée mondiale.</p>
    </article>
</section>
<section>
    <?php
    if ($_ENV['DEBUG_MODE'] === 'true') {
        echo '<h2>Debug</h2>';
        echo '<pre>';
        var_dump($_SESSION);
        echo '</pre>';
    }
    ?>
</section>